...
CI pipeline builds a fresh image from the PR and runs automated tests. Merge →
deploy.
...
The key principle: Build once, deploy everywhere — the same image goes from dev
→ staging → prod, only configuration changes.
